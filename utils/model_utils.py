...
def get_fc1(last_conv, num_classes, fc_type, input_channel=512, bn_mom=cfg.TRAIN.MOMENTUM):
    # body = last_conv
    if fc_type == 'Z':
        body = mx.sym.BatchNorm(data=last_conv, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1')
        ...
        bn1 = mx.sym.BatchNorm(data=last_conv, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1')
        relu1 = act_function(data=bn1, act_type=cfg.TRAIN.NET_ACT, name='relu1')
        ...
        if num_classes > filters_in:
            ...
            body = act_function(data=body, act_type=cfg.TRAIN.NET_ACT, name='convx_relu')
        ...
        conv_6_dw = linear(last_conv, num_filter=input_channel, num_group=input_channel, kernel=(7, 7), pad=(0, 0),
                           bn_mom=bn_mom, stride=(1, 1), name="conv_6dw7_7")
        ...
        fc1 = mx.sym.FullyConnected(data=last_conv, num_hidden=num_classes, name='fc1')
    elif fc_type == 'I':
        body = mx.sym.BatchNorm(data=last_conv, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1')
        fc1 = mx.sym.FullyConnected(data=body, num_hidden=num_classes, name='pre_fc1')
        fc1 = mx.sym.BatchNorm(data=fc1, fix_gamma=True, eps=2e-5, momentum=bn_mom, name='fc1')
    elif fc_type == 'J':
        fc1 = mx.sym.FullyConnected(data=last_conv, num_hidden=num_classes, name='pre_fc1')
        fc1 = mx.sym.BatchNorm(data=fc1, fix_gamma=True, eps=2e-5, momentum=bn_mom, name='fc1')
        pass
    else:
        fc1 = None
        assert fc1 is not None, "fc1 is None"
...
def residual_unit_v3(data, num_filter, stride, dim_match, name, **kwargs):
    ...
    :param num_filter: number of output channels
    :param stride: convolution stride
    :param dim_match: True means channel number between input and output is the same,
                    otherwise means differ
    :param name: base name of the operators
    :param kwargs:
    :return:
    """
    ...
    conv1 = conv_feature(data=bn1, num_filter=num_filter, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                         no_bias=True, workspace=workspace, name=name + '_conv1')
    bn2 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, eps=2e-5, momentum=bn_mom, name=name + '_bn2')
    act1 = act_function(data=bn2, act_type=cfg.TRAIN.NET_ACT, name=name + '_relu1')
    conv2 = conv_feature(data=act1, num_filter=num_filter, kernel=(3, 3), stride=stride, pad=(1, 1),
    ...
    if dim_match:
        ...
        conv1sc = conv_feature(data=data, num_filter=num_filter, kernel=(1, 1), stride=stride, no_bias=True,
...
def residual_unit_v1l(data, num_filter, stride, dim_match, name, bottle_neck):
    ...
    memonger = False
    use_se = cfg.TRAIN.NET_SE
    act_type = cfg.TRAIN.NET_ACT
    # print('in unit1')
    if bottle_neck:
        conv1 = conv_feature(data=data, num_filter=int(num_filter * 0.25), kernel=(1, 1), stride=(1, 1), pad=(0, 0),
                             no_bias=True, workspace=workspace, name=name + '_conv1')
        bn1 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, eps=2e-5, momentum=bn_mom, name=name + '_bn1')
        act1 = act_function(data=bn1, act_type=act_type, name=name + '_relu1')
        conv2 = conv_feature(data=act1, num_filter=int(num_filter * 0.25), kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                             no_bias=True, workspace=workspace, name=name + '_conv2')
        bn2 = mx.sym.BatchNorm(data=conv2, fix_gamma=False, eps=2e-5, momentum=bn_mom, name=name + '_bn2')
        act2 = act_function(data=bn2, act_type=act_type, name=name + '_relu2')
        conv3 = conv_feature(data=act2, num_filter=num_filter, kernel=(1, 1), stride=stride, pad=(0, 0), no_bias=True,
        ...
        if use_se:
            ...
            body = conv_feature(data=body, num_filter=num_filter // 16, kernel=(1, 1), stride=(1, 1), pad=(0, 0),
                                name=name + "_se_conv1", workspace=workspace)
            body = act_function(data=body, act_type=act_type, name=name + '_se_relu1')
            body = conv_feature(data=body, num_filter=num_filter, kernel=(1, 1), stride=(1, 1), pad=(0, 0),
        ...
        if memonger:
            shortcut._set_attr(mirror_stage='True')
        return act_function(data=bn3 + shortcut, act_type=act_type, name=name + '_relu3')
    else:
        conv1 = conv_feature(data=data, num_filter=num_filter, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                             no_bias=True, workspace=workspace, name=name + '_conv1')
        ...
        if memonger:
            shortcut._set_attr(mirror_stage='True')
        return act_function(data=bn2 + shortcut, act_type=act_type, name=name + '_relu3')
    pass
...
def get_head(data, version_input, num_filter):
    ...
    if version_input == 0:
        body = conv_feature(data=data, num_filter=num_filter, kernel=(7, 7), stride=(2, 2), pad=(3, 3),
                            no_bias=True, name="conv0", workspace=workspace)
        body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn0')
        body = act_function(data=body, act_type=act_type, name='relu0')
        body = mx.sym.Pooling(data=body, kernel=(3, 3), stride=(2, 2), pad=(1, 1), pool_type='max')
    else:
        body = data
        _num_filter = min(num_filter, 64)
        body = conv_feature(data=body, num_filter=_num_filter, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                            no_bias=True, name="conv0", workspace=workspace)
        body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn0')
        body = act_function(data=body, act_type=act_type, name='relu0')
